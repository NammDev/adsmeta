...
import { SuspenseWrapper } from '../loading/suspense-wrapper'
...
function ProductsContent() {
  ...
  const filteredProducts =
    currentFilter === 'all' ? allProducts : filterProductSectionItems(currentFilter)
  ...
  const renderProductCard = (product: Product) => (
    <Card
      className={`border-0 rounded-xl overflow-hidden bg-gradient-to-br ${
        product.bgGradient || 'from-blue-50 to-indigo-50'
      } shadow-md hover:shadow-lg transition-all duration-300 h-full transform ${
      ...
      <CardContent className={`${isMobile ? 'p-4' : 'p-6'}`}>
        ...
        <div className='flex justify-between items-center mt-2'>
          ...
          <Button
            ...
            className={`bg-gradient-to-r ${
              product.gradient || 'from-blue-500 to-indigo-500'
            } hover:opacity-90 text-white border-0 shadow-sm`}
            onClick={(e) => {
          ...
          >
            {isLoading
              ...
              : filteredProducts.slice(0, 6).map((product) => (
                  <div
                    key={product.id}
                    className='min-w-[150px] w-[calc(50%-6px)] flex-none snap-start'
                  >
