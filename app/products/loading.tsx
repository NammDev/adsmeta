    ...
    <div className="min-h-screen py-10 sm:py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="flex overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap sm:justify-center gap-4 mb-8 sm:mb-12">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-8 sm:h-10 bg-gray-200 rounded-md w-24 sm:w-28 flex-shrink-0 animate-pulse"></div>
          ))}
        </div>
        ...
        <div className="flex justify-center mt-8 sm:mt-12">
          <div className="flex items-center space-x-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-8 w-8 sm:h-10 sm:w-10 bg-gray-200 rounded-md animate-pulse"></div>
            ))}
          </div>
        </div>
      </div>
